...
var ngSuggest = function(fieldID, params) {
    ...
    if (!this.opts.iMinLen)			this.opts.iMinLen	= 3;						// input: minimal len for search
...
ngSuggest.prototype.callAJAX = function() {
    var pointer = this;
    var params = this.outputGenerator(this);
    if (params.length < this.opts.iMinLen) {
        this.suggestShow(false);
        return;
    }
    $.reqJSON(this.opts.postURL, this.opts.reqMethodName, params, function(json) {
        ...
        if (!json.status) {
            ...
            if (json.recID)
                document.getElementById('re.row.'+json.recID).style.background = '#AAAAAA';
            // ERROR. Display it
            alert('Error ('+json.errorCode+'): '+json.errorText);
        } else {
            //alert('Request complete');

            var sInput	= json.data[0];
            var sReturn	= json.data[1];
