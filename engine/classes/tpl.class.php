...
class tpl {
	...
	function template($name, $dir, $file = '', $params = array()) {
		...
		if (!$dir)
			$dir = defined('ADMIN') ? tpl_actions : tpl_site;
	...
	function vars($nn, $vars = array(), $params = array()) {
		global $userROW, $CurrentHandler, $config, $PHP_SELF, $twig;
		...
		if (preg_match_all('/(?<=\{)l_(.*?)(?=\})/i', $data, $larr)) {
			...
			foreach ($larr[0] as $k => $v) {
				...
				$data = str_replace('{'.$v.'}', (__($name_larr) ? __($name_larr) : '[LANG_LOST:'.$name_larr.']'), $data);
			}
		}
