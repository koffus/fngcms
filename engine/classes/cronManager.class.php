...
class cronManager {
	...
	function run($isSysCron = false) {
		...
		if (sizeof($this->config) == 0) { return 0; }
		...
		while (false !== ($file = readdir($dir))) {
			if ((false !== ($fsize = filesize($cacheDir.'/'.$file))) && (preg_match('#^cron_(\d+)$#', $file, $m))) {
				if ($fsize && (intval($m[1]) > $lastRunTime )) { $lastRunTime = intval($m[1]); }
				else if (intval($m[1]) > $fn_progress) { $fn_progress = intval($m[1]); }
			}
		}
		closedir($dir);
		...
		if (!($dir = opendir($cacheDir))) { return -1; }
		while (false !== ($file = readdir($dir))) {
			if (preg_match("#^cron_(\d+)$#", $file, $m) && intval($m[1])>$fn_max) { $fn_max = $m[1]; }
		}
		closedir($dir);
		...
		foreach ($this->config as $cronLine) {
			...
			foreach ($execList as $rec) {
				...
				// Process line
				if ($rec['min']		!== '*')	{ if ($rec['min']<$at['tm_min'] )		{ $at['tm_hour']++; } $at['tm_min']		= $rec['min'];		}
				if ($rec['hour']	!== '*')	{ if ($rec['hour']<$at['tm_hour'])		{ $at['tm_mday']++; } $at['tm_hour']	= $rec['hour'];	}
				if ($rec['day']		!== '*')	{ if ($rec['day']<$at['tm_mday'])		{ $at['tm_mon']++; } $at['tm_mday']	= $rec['day'];		}
				if ($rec['month']	!== '*')	{ if ($rec['month']<($at['tm_mon']+1))	{ $at['tm_year']++; } $at['tm_mon']		= $rec['month']-1;	}
		...
		if (!($dir = opendir($cacheDir))) { return -1; }
		while (false !== ($file = readdir($dir))) {
			if (preg_match("#^cron_(\d+)$#", $file, $m) && (intval($m[1])<$myFlagTime)) { unlink($cacheDir.'/'.$file); }
		}
		closedir($dir);
