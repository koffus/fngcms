                        ...
                        <div class="panel panel-default panel-table">
                            ...
                            <div class="panel-body table-responsive">
                                <table class="table table-condensed">
                                    <thead>
                                        <tr><th>#ID</th><th>{{ lang['description'] }}</th><th>{{ lang['access'] }}</th></td>
                                    </thead>
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ entry.title }}</td>
                                            <td width="20%">
                                                <select name="{{ entry.name }}|{{group.id}}" onchange="onUpdatePerm('{{ entry.name }}|{{group.id}}');" value="{% if isSet(entry.perm[group.id]) %}{% if (entry.perm[group.id]) %}1{% else %}0{% endif %}{% else %}-1{% endif %}" class="form-control">
...
function onUpdatePerm(name) {
    ...
    if (f[name].value != v) {
        f[name].className = 'pChanged form-control';
    } else {
        f[name].className = 'form-control';
    }
}
