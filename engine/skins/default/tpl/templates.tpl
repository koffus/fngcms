...
var ngFileTreeFunc = function(file) {
    ...
    var method = 'admin.templates.getFile';
    var params = {'token': '{{ token }}', 'template': ngTemplateName, 'file': file, 'content': $('#fileEditorSelector').val()};
    $.reqJSON(url, method, params, function(json) {

        ngFileContent = '';
        ngFileType = json.type;

        $('#fileEditorInfo').html(((ngFileType == 'image')?'Image':'File')+' name: <b>'+ngFileName+'</b> ('+formatSize(json.size)+' )<br/>Last change time: '+json.lastChange);
        ...
        if (json.type == 'image') {
            ...
            $('#imageViewContainer').html(json.content);
        } else {
            ...
            $('#fileEditorSelector').val(json.content);
            // Install codemirror
            var tmode;
            ...
            ngFileContent = json.content;
            emmetCodeMirror(cm);
        }
    });
    $('html, body').animate({scrollTop: $('#templates').offset().top - 50}, '888');
...
function submitTemplateEdit() {
    var url = '{{ admin_url }}/rpc.php';
    var method = 'admin.templates.updateFile';
    var params = {'token': '{{ token }}', 'template': ngTemplateName, 'file': ngFileName, 'content': $('#fileEditorSelector').val()};
    $.reqJSON(url, method, params, function(json) {$.notify({message:json.content},{type: 'info'});});
}
