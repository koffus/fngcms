...
function manage_delete($type){
    ...
    if ($ok and !$err) {
        msg(array('message' => sprintf(__('msgo_deleted'), $ok)));
    } elseif ($ok and $err) {
        msg(array('type' => 'danger', 'message' => sprintf(__('msge_deleted'), $ok, $ok+$err)));
    } elseif (!$ok and !$err) {
        msg(array('type' => 'danger', 'message' => __('msge_nodel')));
    }
}
...
function manage_move($type){
    ...
    if ($files and is_dir($dir.$category) and is_writable($dir.$category)) {
        foreach ($files as $file) {
        ...
        msg(array('message' => __('msgo_moved')));
    } else {
        msg(array('type' => 'danger', 'message' => __('msge_move')));
    }
}

// Manage file/image action
function manage_upload($type){
    global $config, $mysql, $fmanager;
    ...
    if ($subdirectory == "default" and !is_dir($dir.$subdirectory)) {
    ...
    if (!is_dir($dir.$subdirectory)) {
        msg(array('type' => 'danger', 'message' => __('msge_catnexists').' ('.$subdirectory.')'));
        return;
    }
    ...
    foreach($filelist as $fparam) {
            ...
            if ('image' == $type) {
                ...
                if (is_array($sz = $imanager->get_size($dir.$subdirectory.'/'.$up[1]))) {
                    ...
                    $thumb_size_x = 0;
                    $thumb_size_y = 0;
                    if (is_array($thumb) and is_readable($dir.$subdirectory.'/thumb/'.$up[1]) and is_array($szt = $imanager->get_size($dir.$subdirectory.'/thumb/'.$up[1]))) {
                        $thumb_size_x = $szt[1];
                        $thumb_size_y = $szt[2];
                    }
...
function manage_showlist($type) {
    global $config, $mysql, $tpl, $mod, $userROW, $fmanager, $PHP_SELF;
    ...
    // Fill params
    $area = !empty($_REQUEST['area']) ? secure_html($_REQUEST['area']) : 'content';
    $ifield = !empty($_REQUEST['ifield']) ? secure_html($_REQUEST['ifield']) : 'content';

    $author = !empty($_REQUEST['author']) ? secure_html($_REQUEST['author']) : '';
    $category = !empty($_REQUEST['category']) ? secure_html($_REQUEST['category']) : '';
    $postdate = !empty($_REQUEST['postdate']) ? abs(intval($_REQUEST['postdate'])) : '';

    $page = $cstart = !empty($_REQUEST['page']) ? abs(intval($_REQUEST['page'])) : 1;
    $npp = !empty($_REQUEST['npp']) ? abs(intval($_REQUEST['npp'])) : intval($admCookie[$type]['pp']);
    $start_from = !empty($_REQUEST['start_from']) ? abs(intval($_REQUEST['start_from'])) : ($cstart - 1) * $npp;

    if (($npp < 1) or ($npp > 500)) $npp = 20;
    ...
    $filter = ('image' == $type) ? array('category = 0') : array();
    ...
    if ($postdate and (preg_match('/^(\d{4})(\d{2})$/', $postdate, $match)))
    ...
    $where = (count($filter) ? "WHERE ".join(" AND ",$filter) : '');
    $limit = $start_from . ", " . $npp;

    $nCount = 0;
    $entries = '';
    $rows = $mysql->select("SELECT * FROM " . prefix."_".$fmanager->tname . " " . $where . " ORDER BY date desc LIMIT " . $limit);
    foreach ($rows as $row) {
        $nCount++;
        ...
        $tvars['vars'] = array(
            'view_thumb' => (isset($row['preview']) and isset($row['view_thumb'])) ? $row['view_thumb'] : '',
            'file_link' => ('image' != $type) ? '<a href="' . $fileurl . '" title="' . $row['name'] . '" target="_blank">' . $row['orig_name'].'</a> ' : '',
            'file_name' => $row['orig_name'],
            'id' => $row['id'],
            'width' => $row['width'],
            'height' => $row['height'],
            'preview_size' => (('image' == $type) and $row['p_width'] and $row['p_height'])?$row['p_width'].' x '.$row['p_height']:'',
            'preview_img' => (('image' == $type) and $row['preview'])?'<img src="'.$thumburl.'"/><br/>':'',
            'p_width' => $row['p_width'],
        ...
        if ('image' == $type) {
            $tvars['vars']['insert_file'] = '<a href="javascript:insertimage(\''.$html_file.'\', \''.$ifield.'\')" title="Insert file"><img src="'.skins_url.'/images/insert_image.png" border="0"/></a>';
            $tvars['vars']['insert_thumb'] = $row['preview'] ? '<a href="javascript:insertimage(\''.$html_thumb.'\', \''.$ifield.'\')"><img src="'.skins_url.'/images/insert_thumb.png" border="0"/></a>' : '';
            $tvars['vars']['insert_preview'] = $row['preview'] ? '<a href="javascript:insertimage(\''.$html_preview.'\', \''.$ifield.'\')"><img src="'.skins_url.'/images/insert_preview.png" border="0"/></a>' : '';
            ...
            $tvars['vars']['insert_file'] = '<a href="javascript:insertimage(\''.$html_file.'\', \''.$ifield.'\')" class="btn btn-default"><i class="fa fa-paperclip"></i></a>';

        }

        $tvars['regx']['#\[preview\](.+?)\[/preview\]#is'] = $_COOKIE['img_preview']?'$1':'';

        if (('image' == $type) and ($row['preview'])) {
...
//			$tvars['vars']['preview_size'] = '';
    ...
    foreach ($mysql->select("SELECT DISTINCT FROM_UNIXTIME(date,'%Y%m') as monthes, COUNT(date) AS cnt FROM ".prefix."_".$fmanager->tname." GROUP BY monthes ORDER BY date DESC") as $row) {
     if (preg_match('/^(\d{4})(\d{2})$/', $row['monthes'], $match)) {
        $dateslist .= '<option value="'.$row['monthes'].'"'.($row['monthes'] == $postdate ? ' selected' : '').'>'.$langMonths[$match[2]-1].' '.$match[1].'</option>';
     }
    }
    ...
    if ($userROW['status'] == 4) {
        ...
        foreach ($mysql->select("SELECT user, owner_id, count(id) cnt FROM ".prefix."_".$fmanager->tname." WHERE (linked_ds = 0) AND (linked_id = 0) group by owner_id order by user") as $row) {
            $authorlist .= '<option value="'.$row['user'].'"'.($row['user'] == $author ? ' selected':'').'>'.$row['user'].'('.$row['cnt'].')</option>';
        }
    }

    $pagesss = '';
    if (is_array($pcnt = $mysql->record("SELECT count(*) AS cnt FROM " . prefix . "_" . $fmanager->tname . " " . $where))) {
        $itemCount = $pcnt['cnt'];
        $pagesCount = ceil($itemCount / $npp);

        if ($pagesCount) {
            $paginator = new Paginator;
            $pagesss = $paginator->get(
                array(
                    'current' => $cstart,
                    'count' => $pagesCount,
                    'url' => admin_url.'/admin.php?mod=' . $type . 's&action=list'.
                        ($npp ? '&npp=' . $npp : '').
    ...
    // Check if dir exists
    $dName = ('image' == $type) ? images_dir : files_dir;
    if ( !is_dir($dName) or !is_readable($dName) ) {
        msg(array(
        'type' => 'danger',
        'title' => str_replace('{dirname}', $dName, __('error.dir.'.$type.'s')),
        'message' => str_replace('{dirname}', $dName, __('error.dir.'.$type.'s#desc'))),
        1);
        ...
        $dirlistcat = ListDirs($type.'s', $category);
    }

    // Prepare list of available extensions
    $listExt = '';
    foreach (preg_split('#, *#', $config[('image' == $type?'images':'files').'_ext']) as $eI) {
        $listExt .= '*.'.$eI.';';
    }
    ...
    $tvars['vars'] = array(
        ...
        'listExt' => $listExt,
        'descExt' => __( 'uploadify_'.($type=='image'?'images':'files') ),
        'maxSize' => intval($config[($type=='image'?'images':'files').'_max_size'] * 1024),
        'area' => (isset($area) and $area) ? $area : '',
...
{
    global $config, $mysql, $tpl, $mod, $userROW, $fmanager;
...
function manage_editApply($type, $id)
{
    ...
    if ($_POST['flagPreview']) {
        ...
        if ($thumb) {
            // If we created thumb - check if we need to transform it
            $stampThumb = ($_POST['flagStamp'] and !$irow['stamp'] )?1:0;
 ...
 msg(array('message' => 'Изображение отредактировано'));
