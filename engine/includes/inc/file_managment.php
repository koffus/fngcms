...
function manage_showlist($type) {
	...
	if (isset($_REQUEST['postdate']) and $_REQUEST['postdate'] and preg_match('/^(\d{4})(\d{2})$/', $_REQUEST['postdate'], $match))
	...
	foreach ($mysql->select($query['sql']) as $row) {
		...
		$tvars['vars'] = array(
			...
			'width' => $row['width'],
			'height' => $row['height'],
			'preview_size' => (($type == 'image') and $row['p_width'] and $row['p_height'])?$row['p_width'].' x '.$row['p_height']:'',
			'preview_img' => (($type == 'image') and $row['preview'])?'<img src="'.$thumburl.'"/><br/>':'',
			'p_width' => $row['p_width'],
...
//			$tvars['vars']['preview_size'] = '';
	...
	if (is_array($pcnt = $mysql->record($query['count']))) {
		...
		if ($pagesCount) {
			...
			$pagesss = $paginator->get( array('current' => $cstart, 'count' => $pagesCount, 'url' => admin_url.'/admin.php?mod='.$type.'s&action=list'.($_REQUEST['npp']?'&npp='.$npp:'').($_REQUEST['author']?'&author='.$_REQUEST['author']:'').($_REQUEST['category']?'&category='.$_REQUEST['category']:'').($_REQUEST['postdate']?'&postdate='.$_REQUEST['postdate']:'').'&page=%page%'));
		}
	}
