<?php
	...
	if (count($_POST) > 0) {
		...
		$response = $wmxi->X15c(
			trim($_POST["masterwmid"]), # ВМ-идентификатор, которому идентификтор slavewmid данным запросом разрешает или запрещает управление своим кошельком slavepurse
			trim($_POST["slavewmid"]), # ВМ-идентификатор, который доверяет идентификтору masterwmid данным запросом управление своим кошельком slavepurse
			trim($_POST["purse"]), # кошелек, принадлежащий идентификатору slavewmid на который устнавливается доверие
			intval($_POST["ainv"]),
			intval($_POST["atrans"]),
			intval($_POST["apurse"]),
