...
class Finance_Acceptor_WM extends Finance_Acceptor {
	...
	function paymentAcceptForm($sum = 0) {
		...
		$tvars['vars']['sum'] = $sum;
		$tvars['vars']['wm_number'] = strtoupper(pluginGetVariable('fin_wm', 'wm_number'));
	...
	function paymentAccept() {
		...
		$checkline = $WM['payee_purse'].$WM['payment_amount'].$WM['payment_no'].$WM['mode'].$WM['sys_invs_no'].$WM['sys_trans_no'].$WM['sys_trans_date'].pluginGetVariable('fin_wm','secret_key').$WM['payer_purse'].$WM['payer_wm'];
		...
		if (strtolower($checksym) != strtolower($WM['hash'])) {
			...
			if ($login) {
				...
				finance_add_money($login, pluginGetVariable('fin_wm', 'balance_no'), $sum, 'Пополнение счета через WebMoney ('.$pType.' по курсу '.$pRating.')');
				return 'OK';
			}
		}
	}
