...
switch ($action) {
	...
	case 'social' : social_config(); break;
	...
	if (isset($_REQUEST['submit'])) {
		...
		if (isset($_REQUEST['url']) and intval($_REQUEST['url']) == 1) {
			...
			$ULIB->registerCommand('guestbook', '', array(
				...
				'descr' => array(
					'russian' => 'Гостевая книга',
					),
				));
			$ULIB->registerCommand('guestbook', 'edit', array(
				'vars' => array(
					'id' => array(
						'matchRegex' => '\\d+',
						'descr' => array(
...
function modify() {
	...
	msg(array('type' => 'info', 'message' => sprintf($msg, $id)));
}
