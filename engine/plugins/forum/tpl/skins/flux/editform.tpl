		...
		<form name="sendForm" id="sendForm" method="post" action="" enctype="multipart/form-data"> 
			<div class="inform"> 
				<fieldset> 
					...
					<div class="infldset txtarea">
						{% if (subject.true) %}
						<label>Заголовок<br /> 
						<input class="longinput" type="text" name="subject" size="80" maxlength="70" tabindex="1" value="{{ subject.print }}" /><br /></label>
						{% endif %}
						<label><strong>Сообщение</strong><br /> 
						<textarea name="message" id="message" rows="20" cols="95" tabindex="1">{{ message.print }}</textarea><br /></label> 
					</div> 
				</fieldset> 
			</div>
			<div class="inform">
				<fieldset>
					...
					<div class="infldset">
						<div class="rbox">
							Прикрепленные файлы: (отметьте для удаления)<br />
							{% for entry in list_attach %}
							<input type="checkbox" name="attach_delete[]" value="{{ entry.file_id }}" />Удалить?
							...
							Примечание: при использовании кнопки "Просмотр" все опции для файлов не сохраняются, вам придется снова указать какие файлы надо загрузить / какие удалить
						</div>
					</div>
				</fieldset>
			</div>
			<div class="inform">
				<fieldset>
					...
					<div class="infldset">
						<div class="rbox">
							<label><input type="checkbox" name="subscribe" value="1" tabindex="4" />Подписаться и следить за ответами в этой теме<br /></label>
						</div>
					</div>
				</fieldset>
			</div>
			<p><input type="submit" name="submit" value="Отредактировать" tabindex="4" accesskey="s" /><input type="submit" name="preview" value="Предпросмотр" tabindex="4" accesskey="s" />
