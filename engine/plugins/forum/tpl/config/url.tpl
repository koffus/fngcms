			...
			<fieldset class="admGroup">
				...
				<table width="100%" border="0" class="content">
					...
					<tr>
						...
						<td class="contentEntry2" valign=top>{% if (showforum_forum) %}<input class="button" onmousedown="javascript:window.location.href='{{admin_url}}/admin.php?mod=extra-config&plugin=forum&action=url&id=showforum&s=0'" value="Выключить" />{% else %}<input class="button" onmousedown="javascript:window.location.href='{{admin_url}}/admin.php?mod=extra-config&plugin=forum&action=url&id=showforum&s=1'" value="Включить" />{% endif %}</td>
					</tr>
					<tr>
						<td class="contentEntry1" valign=top>Чпу для страницы темы<br /></td>
						<td class="contentEntry2" valign=top>{% if (showtopic_forum) %}<input class="button" onmousedown="javascript:window.location.href='{{admin_url}}/admin.php?mod=extra-config&plugin=forum&action=url&id=showtopic&s=0'" value="Выключить" />{% else %}<input class="button" onmousedown="javascript:window.location.href='{{admin_url}}/admin.php?mod=extra-config&plugin=forum&action=url&id=showtopic&s=1'" value="Включить" />{% endif %}</td>
					</tr>
				</table>
			</fieldset>
		</td>
