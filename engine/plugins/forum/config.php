...
function send_forum()
{
    ...
    if (isset($_REQUEST['submit'])) {
        ...
        if (empty($error_text)) {
			...
			');
            $forum_id = $mysql->lastid(prefix . '_forum_forums');
...
function new_news()
{
    ...
    if (isset($_REQUEST['submit'])) {
        ...
        if (empty($error_text)) {
            ...
            if (isset($mail) && $mail) {
                $news_id = $mysql->lastid(prefix . '_forum_news');
                foreach ($mysql->select('SELECT * FROM ' . prefix . '_users') as $row) {
                    ...
                    sendEmailMessage($row['mail'], 'Новая новость', $xs->render($tVars));
                }
            }
