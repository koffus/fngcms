		...
		case 'apply':
			if(!file_exists(files_dir . 'forum')){
				if(!@mkdir(files_dir . 'forum', 0777)){
					msg(array('type' => 'danger', 'message' => "Критическая ошибка <br /> не удалось создать папку ".files_dir . 'forum'), 1);
					$install = false;
				}
			}
			...
			if(!file_exists(confroot.'extras/forum')){
				if(!@mkdir(confroot.'extras/forum', 0777)){
					msg(array('type' => 'danger', 'message' => "Критическая ошибка <br /> не удалось создать папку ".confroot.'extras/forum'), 1);
					$install = false;
				}
			}
