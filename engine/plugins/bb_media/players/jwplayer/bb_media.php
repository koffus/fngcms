	...
	if (preg_match_all("#\[media(\=| *)(.*?)\](.*?)\[\/media\]#is", $content, $pcatch, PREG_SET_ORDER)) {
		...
		foreach ($pcatch as $catch) {
			...
			if ($fileExt == 'pdf') {
				...
				array_push($rdest, '<embed type="application/x-shockwave-flash" src="'.$config['admin_url'].'/plugins/bb_media/players/jwplayer/lib/swf/player.swf" quality="high" allowfullscreen="true" allowscriptaccess="always" flashvars="'.implode('&amp;', $outfkeys).'" '.(implode(" ", $outkeys)).' />');
			}
		}
