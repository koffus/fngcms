...
function plugin_gmanager_install($action) {
	...
	$ULIB->registerCommand('gmanager', '',
		array ('vars' => array(
				'' => array(
					'matchRegex' => '.+?', 
					'descr' => array(
						$config['default_lang'] => __('gmanager:ULIB_main')
					)
				),
				'page' => array(
					'matchRegex' => '\d{1,4}', 
					'descr' => array(
						$config['default_lang'] => __('gmanager:ULIB_page')
					)
				),
			),
	...
	$ULIB->registerCommand('gmanager', 'gallery',
		array ('vars' => array(
				'name' => array(
					'matchRegex' => '.+?', 
					'descr' => array(
						$config['default_lang'] => __('gmanager:ULIB_name')
						)
					),
				'id' => array(
					'matchRegex' => '\d{1,4}', 
					'descr' => array(
						$config['default_lang'] => __('gmanager:ULIB_id')
						)
					),
				'page' => array(
					'matchRegex' => '\d{1,4}', 
	...
	switch ($action) {
		case 'confirm':
			generate_install_page('gmanager', __('gmanager:desc_install'));
			break;
		case 'autoapply':
		case 'apply':
