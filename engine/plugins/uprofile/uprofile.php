...
function uprofile_showProfile($params) {
	...
	$tVars = array(
		...
		'user'			=> array(
			...
			'com'			=>	$urow['com'],
			'status'		=>	$status,
			'last'			=>	($urow['last'] > 0) ? Lang::retDate("l, j Q Y - H:i", $urow['last']) : __('no_last'),
			'reg'			=>	Lang::retDate("j Q Y", $urow['reg']),
...
function uprofile_applyProfile() {
	global $template, $userROW;

	// Check if user is logged in
	if (!is_array($userROW)) {
		msg(array('type' => 'danger', 'message' => __('uprofile')['msge_notlogged']));
		return;
	}
...
function uprofile_editApply(){
	global $mysql, $tpl, $template, $userROW, $auth_db, $config, $PFILTERS, $DSlist;
	...
	if ($_FILES['newavatar']['name']) {
		...
		if (is_array($up)) {
			...
			if (is_array($sz = $imanage->get_size($config['avatars_dir'].$up[1]))) {
				...
				msg(array('type' => 'danger', 'message' => __('uprofile')['msge_damaged']));
				$fmanage->file_delete(array('type' => 'avatar', 'id' => $up[0]));
			}
		}
	}
	...
	$sqlFields = array (
		...
		'info' => (intval($config['user_aboutsize'])? mb_substr($_REQUEST['editabout'], 0, $config['user_aboutsize'], 'UTF-8') : $_REQUEST['editabout'])
	);
	if ($_REQUEST['editpassword'] != '') {
