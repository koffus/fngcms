...
class autokeyword {
	...
	function parse_words()
	{
		...
		foreach( $s as $key=>$val ) {
			if(mb_strlen(trim($val), 'UTF-8') >= $this->wordLengthMin && mb_strlen(trim($val), 'UTF-8') <= $this->wordLengthMax && !in_array(trim($val), $common) && !is_numeric(trim($val))) {
				$k[] = trim($val);
			}
		}
