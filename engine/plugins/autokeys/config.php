...
if ( file_exists($stopFile = $cPlugin->getFolder('autokeys') .'/config/stop-words/' . $config['default_lang'] . '.sw.txt') )
    $stopText  = file_get_contents( $stopFile );
if ( file_exists($allowFile = $cPlugin->getFolder('autokeys') .'/config/allow-words.txt') )
