	...
	foreach ($result as $row){
		...
		if($row['last_time'] > $last_time){
			...
			if($row['users_status'] == 0){
				...
				$admin_url[] = str_replace( array('{url}', '{name}', '{color_start}', '{color_end}'), array($profile_link, $row['users'], $color_start, $color_end), __('k_online')['admin_url']);
				$team_url[] = str_replace( array('{url}', '{name}', '{color_start}', '{color_end}'), array($profile_link, $row['users'], $color_start, $color_end ), __('k_online')['team_url']);
				$num_team++;
				$num_admin++;
				$num_auth++;
			}elseif($row['users_status'] == 2){
				$color_start = '<span style="color:green;">'; $color_end = '</span>';
				$editor_url[] = str_replace( array('{url}', '{name}', '{color_start}', '{color_end}'), array($profile_link, $row['users'], $color_start, $color_end), __('k_online')['editor_url']);
				$team_url[] = str_replace( array('{url}', '{name}', '{color_start}', '{color_end}'), array($profile_link, $row['users'], $color_start, $color_end ), __('k_online')['team_url']);
				$num_team++;
				$num_editor++;
				$num_user++;
				...
				$publicist_url[] = str_replace( array('{url}', '{name}', '{color_start}', '{color_end}'), array($profile_link, $row['users'], $color_start, $color_end), __('k_online')['publicist_url']);
				$team_url[] = str_replace( array('{url}', '{name}', '{color_start}', '{color_end}'), array($profile_link, $row['users'], $color_start, $color_end ), __('k_online')['team_url']);
				$num_team++;
				$num_publicist++;
				$num_user++;
				...
				$com_url[] = str_replace( array('{url}', '{name}', '{color_start}', '{color_end}'), array($profile_link, $row['users'], $color_start, $color_end), __('k_online')['com_url']);
				$user_url[] = str_replace( array('{url}', '{name}'), array($profile_link, $row['users']), __('k_online')['user_url']);
				$num_users++;
				$num_com++;
				$num_user++;
				$num_auth++;
			}elseif($row['users_status'] == 5){
				$bot_url[] = str_replace( array('{name}'), array(__('k_online')[$row['users']]), __('k_online')['bot_url']);
				$num_bot++;
			}
		}
		if($row['users_status'] == 0){
			...
			$num_today++;
			$today_users[] = str_replace( array('{url}', '{name}', '{date}', '{color_start}', '{color_end}'), array($profile_link, $row['users'], $last_date, $color_start, $color_end), __('k_online')['today_url']);
		}elseif($row['users_status']==5){
			$last_date = date('H:i:s', intval($row['last_time']));
			$num_today++;
			$today_users[] = str_replace( array('{name}', '{date}'), array(__('k_online')[$row['users']], $last_date), __('k_online')['today_bot_url']);
		}
	}
}

if( is_array($admin_url) )
	$activ_adm = str_replace( array('{name}'), array(implode(", ", $admin_url)), __('k_online')['admin_list']);
if( is_array($team_url) )
	$activ_team = str_replace( array('{name}'), array(implode(", ", $team_url)), __('k_online')['team_list']);
if( is_array($editor_url) )
	$activ_red = str_replace( array('{name}'), array(implode(", ", $editor_url)), __('k_online')['editor_list']);
if( is_array($publicist_url) )
	$activ_pub = str_replace( array('{name}'), array(implode(", ", $publicist_url)), __('k_online')['publicist_list']);
if( is_array($com_url) )
	$activ_com = str_replace( array('{name}'), array(implode(", ", $com_url)), __('k_online')['com_list']);
if( is_array($user_url) )
	$activ_user = str_replace( array('{name}'), array(implode(", ", $user_url)), __('k_online')['user_list']);
if( is_array($bot_url) )
	$activ_bot = str_replace( array('{name}'), array(implode(", ", $bot_url)), __('k_online')['bot_list']);
if(is_array($today_users))
	$was_now = str_replace( array('{name}', '{num_today}'), array(implode(", ", $today_users), $num_today), __('k_online')['today_list']);
