...
<table class="content" border="0" cellspacing="0" cellpadding="0" align="center">
	...
	<tr>
		<td width="100%" class="contentHead"><img src="{admin_url}/plugins/pm/img/nav.gif" hspace="8">{{subject}} {% if (ifinbox) %}от{% endif %} {% if not (ifinbox) %}для{% endif %} {{author}} {{pmdate|date('Y-m-d H:i')}}</td>
	</tr>
	...
	<tr align="center">
		...
		{% if (ifinbox == 1) %}<form name="pm" method="POST" action="{{php_self}}?action=reply&pmid={{pmid}}"><input class="button" type="submit" value="{{ lang['pm:reply'] }}"></form>{% endif %}</td>
	</tr>		
</table>
</div>