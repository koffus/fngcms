...
{global $twig;
	...
	foreach($tVars as $row){
		...
		if(!empty($row['error'])){
			$info .= msg(array('type' => 'danger', 'message' => 'У вас ошибка! Если не получается её исправить, обратитесь на форум <a href="http://ngcms.ru/forum/viewtopic.php?id=2055" target="_blank"><b>simple_title_pro</b></a> или мне на ICQ: 209388634 или jabber: rozard@ya.ru'), 0, 2);
			break;
		}
	}
...
{global $mysql;
	...
	if( empty($id) )
		return msg(array('type' => 'danger', 'message' => 'Вы не выбрали что хотите удалить'));
