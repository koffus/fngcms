...
{
global $tpl, $cron, $mysql, $config, $parse, $PFILTERS;
...
//Email informer
	...
	$body = str_replace(
			...
			__('subscribe_comments:msg.notice')
		);
...
$ends = str_replace(
			...
			__('subscribe_comments:msg.notice_end')
		);
...
function plugin_subscribe_comments() {
	global $config, $mysql, $CurrentHandler;
	global $catmap, $catz, $userROW, $template, $tpl;
...
if(empty($altname)) { $altname = secure_html($_REQUEST['subscribe_news']); }

$email = secure_html($_REQUEST['subscribe_field']);

// msg(array('type' => 'danger', 'message' => __('subscribe_comments:err.password')));

if (!(filter_var($email, FILTER_VALIDATE_EMAIL)) or empty($email)) { msg(array('type' => 'danger', 'message' => __('subscribe_comments:err.wrongemail'))); header('Refresh: '.$timeout.'; URL='.secure_html($_SERVER["HTTP_REFERER"]).''); return; }
...
//var_dump($_REQUEST);
	...
	{
		case '1':
		if (!empty($email) and !empty($altname))
		{
			if(empty($altname)) { msg(array('type' => 'danger', 'message' => __('subscribe_comments:err.wrongparams'))); return; }
			$row = $mysql->record("select * from ".prefix."_news where alt_name='".$altname."'");
			if(empty($row)) { msg(array('type' => 'danger', 'message' => __('subscribe_comments:err.wrongaltname'))); return; }
			...
			msg(array('message' => str_replace(array('{title}'), array($row['title']), __('subscribe_comments:msg.unsubscribepageok')))); return;
			}
			else
			{
			msg(array('type' => 'danger', 'message' => __('subscribe_comments:err.unsubscribepageyet'))); return;
			}
		}
		else {
			msg(array('type' => 'danger', 'message' => __('subscribe_comments:err.wrongparams'))); return;
			 }
		break;
		...
		if (!empty($email))
		{
			...
			msg(array('message' => __('subscribe_comments:msg.unsubscribeall'))); return;
			}
			else
			{
			msg(array('type' => 'danger', 'message' => __('subscribe_comments:err.unsubscribeall'))); return;
			}
		...
		default:
		if(empty($altname)) { msg(array('type' => 'danger', 'message' => __('subscribe_comments:err.wrongparams'))); header('Refresh: '.$timeout.'; URL='.secure_html($_SERVER["HTTP_REFERER"]).''); return; }
		
		$row = $mysql->record("select * from ".prefix."_news where alt_name='".$altname."'");
		if(empty($row)) { msg(array('type' => 'danger', 'message' => __('subscribe_comments:err.wrongaltname'))); header('Refresh: '.$timeout.'; URL='.secure_html($_SERVER["HTTP_REFERER"]).''); return; }
		
		if(!($nrow = $mysql->record("select * from ".prefix."_subscribe_comments where news_id='".$row['id']."' and news_altname='".$row['alt_name']."' and user_email='".$email."'")))
		{
		$mysql->query("insert into ".prefix."_subscribe_comments (user_email, news_id, news_altname) values ('".$email."', '".$row['id']."', '".$row['alt_name']."')");
		msg(array('message' => __('subscribe_comments:msg.subscribepageok'))); header('Refresh: '.$timeout.'; URL='.secure_html($_SERVER["HTTP_REFERER"]).''); return;
		}
		else
		{
		msg(array('message' => __('subscribe_comments:err.subscribepageyet'))); header('Refresh: '.$timeout.'; URL='.secure_html($_SERVER["HTTP_REFERER"]).''); return;
		}
		break;
	}
/*
...
pluginRegisterFilter('comments','subscribe_comments', new SubscribeComments);
pluginRegisterFilter('news','subscribe_comments', new ShowSubscribeForm);
