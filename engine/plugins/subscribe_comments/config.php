...
switch ($action) {
	...
	case 'modify': modify(); if($get_params['action'] == 'list_subscribe') show_list_subscribe(); else show_list_subscribe_post();	break;
	default: main();
}
...
$tvars['regx']["'\[hide_delayed\](.*?)\[/hide_delayed\]'si"] = '';
	...
	$pagesss = $paginator->get(array(
								'current' => $pageNo,
								'count' => $countPages,
								'url' => admin_url.'/admin.php?mod=extra-config&plugin=subscribe_comments&action=list_subscribe'.($_REQUEST['news_per_page']?'&news_per_page='.$news_per_page:'').($_REQUEST['author']?'&author='.$_REQUEST['author']:'').($_REQUEST['sort']?'&sort='.$_REQUEST['sort']:'').($postdate?'&postdate='.$postdate:'').($author?'&author='.$author:'').($status?'&status='.$status:'').'&page=%page%'
								));
...
$tvars['regx']["'\[hide_delayed\](.*?)\[/hide_delayed\]'si"] = '';
	...
	$pagesss = $paginator->get(array(
								'current' => $pageNo,
								'count' => $countPages,
								'url' => admin_url.'/admin.php?mod=extra-config&plugin=subscribe_comments&action=list_subscribe_post'.($_REQUEST['news_per_page']?'&news_per_page='.$news_per_page:'').($_REQUEST['author']?'&author='.$_REQUEST['author']:'').($_REQUEST['sort']?'&sort='.$_REQUEST['sort']:'').($postdate?'&postdate='.$postdate:'').($author?'&author='.$author:'').($status?'&status='.$status:'').'&page=%page%'
								));
...
global $mysql;
	...
	if( empty($id) )
	{
		return msg(array('type' => 'danger', 'message' => 'Вы не выбрали объектов'));
	}
	
	switch($subaction) {
		case 'mass_delete' : $del = true; break;
		case 'mass_delete_post' : $del_post = true; break;
	}

	if(isset($del))
	{
		$mysql->query("delete from ".prefix."_subscribe_comments where id in ({$id})");
		msg(array('message' => "Подписки с ID${id} удалены"));
	}
	
	if(isset($del_post))
	{
		$mysql->query("delete from ".prefix."_subscribe_comments_temp where id in ({$id})");
		msg(array('message' => "Пиьсма с ID${id} удалены"));
	}
}
...
if( $config['syslog'] == '1' ) {
	...
	if($value['plugin']=='subscribe_comments') { $cron_min = $value['min']; $cron_hour = $value['hour']; $cron_day = $value['day']; $cron_month = $value['month']; }
	}
	if(!isset($cron_min)) { $cron_min = '0,5,10,15,20,25,30,35,40,45,50,55'; }
