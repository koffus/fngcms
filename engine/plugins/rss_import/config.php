...
for ($i = 1; $i <= $count; $i++) {
	$cfgX = array(); 
 array_push($cfgX, array('name' => 'rss'.$i.'_name', 'title' => 'Заголовок новостей для отображения<br /><small>Например: <b>Next Generation CMS</b></small>', 'type' => 'input', 'value' => pluginGetVariable($plugin,'rss'.$i.'_name')));	
	array_push($cfgX, array('name' => 'rss'.$i.'_url', 'title' => 'Адрес новостей для отображения<br /><small>Например: <b>http://ngcms.ru</b></small>', 'type' => 'input', 'value' => pluginGetVariable($plugin,'rss'.$i.'_url')));	
	array_push($cfgX, array('name' => 'rss'.$i.'_number', 'title' => 'Количество новостей для отображения<br /><small>Значение по умолчанию: <b>10</b></small>', 'type' => 'input', 'value' => intval(pluginGetVariable($plugin,'rss'.$i.'_number'))?pluginGetVariable($plugin, 'rss'.$i.'_number'):'10'));	
	array_push($cfgX, array('name' => 'rss'.$i.'_maxlength', 'title' => 'Ограничение длины названия новости<br /><small>Если название превышает указанные пределы, то оно будет урезано<br />Значение по умолчанию: <b>100</b></small>', 'type' => 'input', 'value' => intval(pluginGetVariable($plugin, 'rss'.$i.'_maxlength'))?pluginGetVariable($plugin ,'rss'.$i.'_maxlength'):'100'));
	array_push($cfgX, array('name' => 'rss'.$i.'_newslength', 'title' => 'Ограничение длины короткой новости<br /><small>Если название превышает указанные пределы, то оно будет урезано<br />Значение по умолчанию: <b>100</b></small>', 'type' => 'input', 'value' => intval(pluginGetVariable($plugin, 'rss'.$i.'_newslength'))?pluginGetVariable($plugin ,'rss'.$i.'_newslength'):'100'));
	array_push($cfgX, array('name' => 'rss'.$i.'_content', 'title' => "Генерировать переменную {short_news}", 'type' => 'checkbox', value => pluginGetVariable($plugin,'rss'.$i.'_content')));
	array_push($cfgX, array('name' => 'rss'.$i.'_img', 'title' => "Удалить все картинки из {short_news}", 'type' => 'checkbox', value => pluginGetVariable($plugin,'rss'.$i.'_img')));
