...
array_push($cfgX, array('name' => 'dateformat', 'title' => "Формат даты при отображении информации о новости", 'descr' =>"Значение по умолчанию: \"{day0}.{month0}.{year}\"<br/>Доступные переменные:<br/>{day} - день (1 - 31)<br>{day0} - день (01 - 31)<br>{month} - месяц (1 - 12)<br>{month0} - месяц (01 - 12)<br>{year} - год (00 - 99)<br>{year2} - год (1980 - 2100)<br>{month_s} - текст месяца (Янв, Фев,...)<br>{month_l} - текст месяца (Январь, Февраль,...)<br/>{hour} - час (0-23)<br/>{hour0} - час (00-23)<br/>{minute0} - минута (00-59)", 'type' => 'input', 'value' => pluginGetVariable($plugin,'dateformat')));
array_push($cfgX, array('name' => 'number', 'title' => "Кол-во новостей для отображения", 'descr' =>"Значение по умолчанию: <b>10</b>", 'html_flags' => 'size=5', 'type' => 'input', 'value' => pluginGetVariable($plugin,'number')));
array_push($cfgX, array('name' => 'maxlength', 'title' => "Ограничение длины названия новости", 'descr' => "Значение по умолчанию: <b>100</b><br/>(если название превышает указанные пределы, то оно будет урезано)", 'html_flags' => 'size=5', 'type' => 'input', 'value' => intval(pluginGetVariable($plugin,'maxlength'))?pluginGetVariable($plugin,'maxlength'):'100'));
...
array_push($cfgX, array('name' => 'cacheExpire', 'title' => "Период обновления кеша<br /><small>(через сколько секунд происходит обновление кеша. Значение по умолчанию: <b>60</b>)</small>", 'html_flags' => 'size=5', 'type' => 'input', 'value' => intval(pluginGetVariable($plugin,'cacheExpire'))?pluginGetVariable($plugin,'cacheExpire'):'60'));
