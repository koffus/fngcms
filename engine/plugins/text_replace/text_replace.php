...
class Text_ReplaceNewsFilter extends NewsFilter {
	function Text_ReplaceNewsFilter(){
		...
		$this->text = pluginGetVariable('text_replace','replace');
		$this->p_count = pluginGetVariable('text_replace','p_count');
		$this->c_replace = pluginGetVariable('text_replace','c_replace');
		$this->str_url = pluginGetVariable('text_replace','str_url');
	...
	function text_replace(&$text, $search, $replace, $p_count = 3, $pos_s = 0, $i = 1){
		while($i <= $p_count){
			...
			switch($this->c_replace){
				case 0: $pos = mb_stripos($text, $search, $pos_s); break;
				case 1: $pos = mb_stripos($text, $search, $pos_s); break;
				case 2: $pos = strpos($text, $search, $pos_s); break;
				default: $pos = strpos($text, $search, $pos_s);
			}
			...
			if($this->c_replace <> 0){
				if($text{$pos+mb_strlen($search, 'UTF-8')} == '' 
				or $text{$pos+mb_strlen($search, 'UTF-8')} == ' ' 
				or $text{$pos+mb_strlen($search, 'UTF-8')} == '<'){}
				else return $this->text_replace($text, $search, $replace, $p_count, $pos+1, $i-1);
			}
			$text = substr_replace($text, $replace, $pos, mb_strlen($search, 'UTF-8'));
		}
		return $text;
	}
...
pluginRegisterFilter('news', 'text_replace', new Text_ReplaceNewsFilter);