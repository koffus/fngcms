...
function show_comments() {
    ...
    $perpage = pluginGetVariable('show_comments', 'perpage');
    if ($perpage == '') {
        $perpage = "5";
    }

    $order = pluginGetVariable('show_comments', 'order');
    if ($order == 'desc') {
        $order = "DESC";
    } elseif ($order == 'asc') {
        $order = "ASC";
    ...
    foreach ($result as $prd) {
        ...
        echo '<tr>';
        echo '<th>' . Lang::retDate('d.m.Y H:i:s', $prd['postdate']) . ' [<a href="/engine/admin.php?mod=editcomments&newsid=' . $prd['nid'] . '&comid=' . $prd['id'] . '" target="_blank">#</a>] [<a href="/engine/admin.php?mod=editcomments&subaction=deletecomment&newsid=' . $prd['nid'] . '&comid=' . $prd['id'] . '&poster=' . $prd['author'] . '" target="_blank">X</a>]</th>';
        echo '<th>' . $text . '</th>';
        echo '<th><a href="' . News::generateLink(array('id' => $prd['nid'], 'alt_name' => $prd['alt_name'], 'catid' => $prd['catid'], 'postdate' => $prd['npostdate'])) . '" target="_blank">' . str_replace('<', '&lt;', $prd['title']) . '</a> [<a href="/engine/admin.php?mod=news&action=edit&id=' . $prd['nid'] . '" target="_blank">E</a>]</th>';
        if ($prd['author_id']) {
    ...
    if (!empty($type)) {
        ...
        $query = substr($query, 0, mb_strlen($query, 'UTF-8') - 1) . ")";
...
$cfgX = array();
    array_push($cfgX, array(
        'name' => 'perpage',
        'title' => 'Кол-во комментариев для отображения на одной странице',
        'type' => 'input',
        'value' => pluginGetVariable($plugin, 'perpage'),
        ));
    array_push($cfgX, array(
        'name' => 'comm_length',
        'title' => 'Усечение длины комментария',
        'descr' => 'Кол-во символов из комментария для отображения<br/>Значение по умолчанию: <b>50</b>',
        'type' => 'input',
        'value' => pluginGetVariable($plugin, 'comm_length'),
        ));
    array_push($cfgX, array(
        'name' => 'order',
        'title' => 'Упорядочить по:',
        'descr' => 'Выберите порядок отображения комментариев.',
        'type' => 'select',
        'values' => array('asc' => 'Возрастанию', 'desc' => 'Убыванию'),
