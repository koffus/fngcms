...
function plugin_multi_main() {
	...
	// ** MAIN PAGE
	if (pluginGetVariable('multi_main', 'main') and isset($confArray['predefined']['REQUEST_URI']) and $confArray['predefined']['REQUEST_URI'] == '/'){
		$SYSTEM_FLAGS['template.main.name'] = pluginGetVariable('multi_main', 'main');
		return;
	}

	// ** NEWS CATEGORY PAGE
	if($CurrentHandler['pluginName'] == 'news' and $CurrentHandler['handlerName'] == 'by.category'){
		$catname = '';
	...
	if (($CurrentHandler['pluginName'] == 'news') and ($CurrentHandler['handlerName'] == 'news')) {
		$calt = $SYSTEM_FLAGS['news']['currentCategory.alt'];
		$category = pluginGetVariable('multi_main', 'category');
		if (is_array($category) and array_key_exists($calt, $category)){
