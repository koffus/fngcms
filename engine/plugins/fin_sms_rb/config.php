...
array_push($cfgX, array('name' => 'prefix', 'title' => 'SMS-префикс', 'descr' => 'Здесь необходимо указать префикс, на который абоненты должны отправлять свои SMS','type' => 'input', value => pluginGetVariable('fin_sms_rb','passkey')));
array_push($cfg, array('mode' => 'group', 'title' => '<b>Настройки интеграции</b>', 'entries' => $cfgX));

$cfgX = array();
array_push($cfgX, array('name' => 'bonus_mode', 'title' => 'Объём начисления бонуса пользователям', 'descr' => '<b>Ваша прибыль</b>','type' => 'select', 'values' => array( '1' => 'Ваша прибыль'), value => pluginGetVariable('fin_sms_rb','bonus_mode')));
array_push($cfgX, array('name' => 'balance_no', 'title' => 'Номер баланса на который начисляются бонусы', 'descr' => '<b>Общая сумма</b> - начисляется общая сумма, списанная с его счёта<br /><b>Ваша прибыль</b> - начисляется сумма, которую получите вы<br /><font color=red>В списке указываются <u>только</u> монетарные балансы!</font>','type' => 'select', 'values' => $blist, value => pluginGetVariable('fin_sms_rb','balance_no')));
