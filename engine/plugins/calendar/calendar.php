...
if (!pluginGetVariable('calendar', 'mode')) {
    registerActionHandler('index', 'plugin_calendar');
} else {
    global $template;
...
function plug_calgen($month, $year, $overrideTemplateName = false, $categoryList = array(), $cacheExpire = 0, $flagAJAX = false)
{
    ...
    foreach (explode(',', __('short_weekdays')) as $k => $v) {
        Lang::set(array('weekday_' . $k => $v));
...
function plugin_calendar_showTwig($params)
{
    ...
    // Check offset
    if (isset($params['offset']) and ($params['offset'] == 'prev')) {
        $month--;
    }
    if (isset($params['offset']) and ($params['offset'] == 'next')) {
        $month++;
    }
