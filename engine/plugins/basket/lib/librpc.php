...
function basket_add_item($linked_ds, $linked_id, $title, $price, $count, $xfld = array()) {
    ...
    $mysql->query("insert into ".prefix."_basket (
        user_id,
        cookie,
        linked_ds,
        linked_id,
        ...
        ".(is_array($userROW)?db_squote($userROW['id']):0).",
        ".db_squote($_COOKIE['ngTrackID']).",
        ".db_squote($linked_ds).",
        ".db_squote($linked_id).",
        ".db_squote($title).",
        ".db_squote(serialize($xfld)).",
        ".db_squote($price).",
        ".db_squote($count).
        ") on duplicate key update price=".db_squote($price).", count = count+".db_squote($count));
                ...
                case $DSlist['news']:
                    ...
                    foreach ($xc['news'] as $k => $v) {
                        if(isset($xfData[$k])) {
                            ...
                            return array('status' => 0, 'errorCode' => 3, 'errorText' => '<b>' . $k . '</b> to this Item not found ');
                        }
                    }
