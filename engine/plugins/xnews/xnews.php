...
function xNewsShowBlock($params) {
	...
	if (isset($params['id']) and $params['id']) {
		...
		for ($i = 1; $i < 50; $i++) {
			if (pluginGetVariable('xnews', $i.'_name') == $params['id']) {
				...
				if (!isset($params['template']) and (pluginGetVariable('xnews', $i.'_template') != ''))
	...
	if (isset($params['template'])) {
		if (is_dir(tpl_site.'plugins/xnews/'.$params['template']) and file_exists(tpl_site.'plugins/xnews/'.$params['template'].'/entries.tpl') and file_exists(tpl_site.'plugins/xnews/'.$params['template'].'/xnews.tpl')) {
			// TEMPLATE FOUND: SITE
			$templateDir = tpl_site.'plugins/xnews/'.$params['template'];
		} else if (is_dir(extras_dir.'/xnews/tpl/'.$params['template']) and file_exists(extras_dir.'/xnews/tpl/'.$params['template'].'/entries.tpl') and file_exists(extras_dir.'/xnews/tpl/'.$params['template'].'/xnews.tpl')) {
		...
		if (is_dir(extras_dir.'/xnews/tpl/example') and file_exists(extras_dir.'/xnews/tpl/example/entries.tpl') and file_exists(extras_dir.'/xnews/tpl/example/xnews.tpl')) {
	...
	if (($showResult['count'] < 1) and (isset($params['showNoNews']) or ($params['showNoNews']))) {
		$xOut = '';
	}
