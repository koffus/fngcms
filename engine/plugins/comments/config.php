...
$cfgX = array();
	...
	array_push($cfgX, array(
		...
		'descr' => '<b>Да</b> - гости не смогут в качестве email адреса указывать адрес уже зарегистрированных пользоваталей<br/><b>Нет</b> - гость может использовать любой валидный email адрес',
		'type' => 'select',
		'values' => array('0' => __('noa'), '1' => __('yesa')),
	...
	array_push($cfgX, array(
		...
		'descr' => "<b>Прямая</b> - отображение в порядке добавления<br/><b>Обратная</b> - самые новые показываются первыми",
		'type' => 'select',
	...
	array_push($cfgX, array(
		...
		'descr' => "Укажите максимальное кол-во символов для комментариев (например: <b>200</b>; <b>0</b> - не ограничивать)",
		'type' => 'input',
	...
	array_push($cfgX, array(
		...
		'descr' => "В случае превышения заданного числа, в слово будет автоматически будет добавляться пробел (например: <b>50</b>)",
		'type' => 'input', 'value' => pluginGetVariable($plugin, 'maxwlen'),
		));
	array_push($cfgX, array(
		'name' => 'multi',
		'title' => "Разрешить множественные комментарии",
		'descr' => "<b>Да</b> - пользователь может оставлять последовательно несколько комментариев<br/><b>Нет</b> - пользователю запрещено размещать последовательно несколько комментариев (необходимо дождаться комментария другого пользователя)",
		'type' => 'select',
		'values' => array('0' => __('noa'), '1' => __('yesa')),
	...
	array_push($cfgX, array(
		...
		'descr' => "<b>Да</b> - автор может оставлять последовательно несколько комментариев<br/><b>Нет</b> - автору запрещено размещать последовательно несколько комментариев",
		'type' => 'select',
		'values' => array('0' => __('noa'), '1' => __('yesa')),
	...
	array_push($cfgX, array(
		...
		'descr' => 'Помощь по работе функции: <a href="http://php.net/date/" target="_blank">php.net/date</a><br/>Значение по умолчанию: <b>j.m.Y - H:i</b>',
		'type' => 'input',
...
$cfgX = array();
	...
	array_push($cfgX, array(
		...
		'descr' => 'При добавлении новостей по умолчанию будет устанавливаться:<br/><b>запретить</b> - комментарии будут запрещены<br/><b>разрешить</b> - комментарии будут разрешены<br/><b>по умолчанию</b> - флаг разрешения/запрета комментариев будет браться из настроек главной категории новости',
		'type' => 'select',
...
$cfgX = array();
	array_push($cfgX, array(
		...
		'descr' => '<b>Да</b> - на странице новости будет отображаться только часть комментариев, остальные будут доступны на отдельной страничке<br/><b>Нет</b> - все комментарии будут отображаться на странице новости',
		'type' => 'select',
		'values' => array('0' => __('noa'), '1' => __('yesa')),
	...
	array_push($cfgX, array(
		...
		'descr' => "Укажите кол-во комментариев, отображаемых на странице новости<br/>(<b>0</b> - не отображать ни одного комментария)",
		'type' => 'input',
	...
	array_push($cfgX, array(
		...
		'descr' => "Укажите кол-во комментариев, отображаемых на каждой странице с комментариями<br/>(<b>0</b> - отображать все на одной странице)",
		'type' => 'input',
...
$cfgX = array();
	array_push($cfgX, array(
		...
		'descr' => "<b>Да</b> - при добавлении каждого комментария автор будет получать e-mail сообщение<br/><b>Нет</b> - автор не будет получать e-mail нотификаций",
		'type' => 'select',
		'values' => array('0' => __('noa'), '1' => __('yesa')),
	...
	array_push($cfgX, array(
		...
		'descr' => "<b>Да</b> - при добавлении каждого комментария администратор будет получать e-mail сообщение<br/><b>Нет</b> - администратор(ы) не будет получать e-mail нотификаций",
		'type' => 'select',
		'values' => array('0' => __('noa'), '1' => __('yesa')),
