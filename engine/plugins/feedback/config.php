...
function saveForm()
{
    ...
    if (!is_array($recF = $mysql->record("select * from " . prefix . "_feedback where id = " . $id))) {
        msg(array('type' => 'danger', 'message' => 'Указанная вами форма не существует'));
        showForm(1);
        return;
    }
    ...
    if ($name == '') {
        msg(array('type' => 'danger', 'message' => 'Необходимо заполнить ID формы'));
        showForm(1);
        return;
    }
    ...
    if (is_array($mysql->record("select * from " . prefix . "_feedback where id <> " . $id . " and name =" . db_squote($name)))) {
        msg(array('type' => 'danger', 'message' => 'Форма с таким ID уже существует. Нельзя использовать одинаковый ID для разных форм'));
        showForm(1);
        return;
    }
...
function showFormRow()
{
    ...
    do {
        ...
        foreach (array('text', 'email', 'textarea', 'select', 'date') as $ts) {
            ...
            $xsel .= '<option value="' . $ts . '"' . (($xRow['type'] == $ts) ? ' selected' : '') . '>' . __('feedback:type.' . $ts);
        }
