...
<ul class="breadcrumb">
	...
	<li class="active">{% if (flags.addField) %}Добавление нового поля{% else %}Поле "{{ fieldName }}"{% endif %}</li>
</ul>
...
<div class="page-main">
	...
	<form action="?mod=extra-config&plugin=feedback&action=editrow&form_id={{ formID }}&name={{ fieldName }}" method="post" name="feedbackform">
		...
		<tr class="contRow1"><td width="50%">Блокировка:<br/><small><b>Нет</b> - поле доступно для редактирования<br/><b>Да</b> - пользователь не может редактировать данное поле<br/><b>Да, невидимо</b> - поле невидимо, его редактирование запрещено</small></td><td>
		<select name="block">
