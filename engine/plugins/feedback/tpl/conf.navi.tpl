...
<td colspan="2" width=100% class="contentHead">
	...
	{% if (flags.haveForm) %}
	&#8594;	<a href="?mod=extra-config&plugin=feedback&action=form&id={{ formID }}">Форма "{{ formName }}"</a>
	{% if (flags.haveField) %}
	&#8594;	<a href="?mod=extra-config&plugin=feedback&action=row&form_id={{ formID }}&row={{ fieldName }}">Поле "{{ fieldName }}"</a>
	{% endif %}
	{% if (flags.addField) %}
