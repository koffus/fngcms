...
<tr align="left" class="contRow1">
	...
	<td nowrap>{% if (entry.flags.active) %}<a onclick="alert('{{ lang['feedback:active_nodel'] }}');">{% else %}<a href="{{ entry.linkDel }}" onclick="return confirm('{{ lang['feedback:suretest'] }}');">{% endif %}<img src="{{ skins_url }}/images/delete.gif" alt="DEL" width="12" height="12" /></a></td>
</tr>
{% endfor %}
<tr>
<td></td>
