...
function plugin_feedback_showScreen($mode = 0, $errorText = '') {
	global $template, $mysql, $userROW, $PFILTERS, $twig, $SYSTEM_FLAGS;
	...
	if (!is_array($frow = $mysql->record("select * from ".prefix."_feedback where active = 1 and id = ".$form_id))) {
		$tVars = array(
			'title' => __('feedback:form.no.title'),
			'ptpl_url' => $ptpl_url,
	...
	if ($link_news > 0) {
		...
		if (!$linked_id or !is_array($nrow = $mysql->record("select * from ".prefix."_news where (id = ". db_squote($linked_id) . ") and (approve = 1)"))) {
			...
			if ($link_news == 2) {
				...
				$tVars = array(
					'title' => __('feedback:form.nolink.title'),
					'ptpl_url' => $ptpl_url,
...
function plugin_feedback_post() {
	...
	$SYSTEM_FLAGS['info']['title']['item']	= str_replace('{title}', $frow['title'], __('feedback:header.send'));
	...
	$mailSubject = str_replace(array('{name}', '{title}'), array($frow['name'], $frow['title']), $flagSubj?$frow['subj']:__('feedback:mail.subj'));
