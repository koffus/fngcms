        ...
        if (pluginGetVariable('neighboring_news', 'full_mode') and $mode['style'] == 'full') {
            ...
            if (intval($SQLnews['catid']) != null) {
                ...
                if ($row['alt_name']) {
                    ...
                    $tpl->vars('next_news', array('vars' => array(
                        'link' => News::generateLink(array('id' => $row['id'], 'alt_name' => $row['alt_name'], 'catid' => $row['catid'], 'postdate' => $row['postdate']), false, 0, true),
                ...
                if ($row['alt_name']) {
                    ...
                    $tpl->vars('previous_news', array('vars' => array(
                        'link' => News::generateLink(array('id' => $row['id'], 'alt_name' => $row['alt_name'], 'catid' => $row['catid'], 'postdate' => $row['postdate']), false, 0, true),
                        'date' => Lang::retDate('d.m.Y', $row['postdate']),
                        'author' => $author_link,
                        'title' => str_replace(array("'", "\""), array("&#039;", "&quot;"), $row['title'])
                ...
                $tpl->vars('neighboring_news', array('vars' => array(
                    'next_news' => $next_news,
