...
function plugin_nsm(){
 ...
 if (!is_array($userROW) or !$permPlugin['view']) {
 ...
 (($row['approve'] == 1)&&(!$permPlugin['view.published']))) and (!$permPlugin['list'])) {
 continue;
 }
 
 $cats = (strlen($row['catid'])>0)?explode(',', $row['catid']):array();

 $canView = ((($row['approve'] == -1) and ($permPlugin['view.draft'])) ||
 (($row['approve'] == 0) and ($permPlugin['view.unpublished'])) ||
 (($row['approve'] == 1) and ($permPlugin['view.published']))) and $perm['personal.view'];

 $canEdit = (($row['approve'] == -1) and ($permPlugin['modify.draft'])) ||
 (($row['approve'] == 0) and ($permPlugin['modify.unpublished']) and ($perm['personal.modify'])) ||
 (($row['approve'] == 1) and ($permPlugin['modify.published']) and ($perm['personal.modify.published']));

 $canDelete = (($row['approve'] == -1) and ($permPlugin['delete.draft'])) ||
 (($row['approve'] == 0) and ($permPlugin['delete.unpublished']) and ($perm['personal.delete'])) ||
 (($row['approve'] == 1) and ($permPlugin['delete.published']) and ($perm['personal.delete.published']));
 ...
 'comments' => $cPlugin->isActive('comments')?true:false,
...
function plugin_nsm_add($tpl_name){
 ...
 if ( isset($_REQUEST['mod']) and ($_REQUEST['mod'] == 'preview') ) {
...
function plugin_nsm_edit($tpl_name){
 global $mysql, $userROW, $SUPRESS_TEMPLATE_SHOW;
 ...
 msg(array('type' => 'danger', 'message' => __('nsm')['err.news_not_found']));
 return;
 }

 // We can manage only OWN news
 if ((!is_array($userROW)) or ($row['author_id'] != $userROW['id'])) {
 msg(array('type' => 'danger', 'message' => __('perm.denied')));
 return 0;
 }
...
function plugin_nsm_addForm($tpl_name = 'news.add', $retry = ''){
 global $userROW, $twig, $template, $config, $PHP_SELF, $catz;
...
function plugin_nsm_editForm($tpl_name = 'news.edit',$retry = ''){
 global $parse, $mysql, $config, $PFILTERS, $tvars, $userROW, $twig, $template, $catz;
...
function plugin_nsm_del(){
 ...
 msg(array('type' => 'danger', 'message' => 'xx' . __('perm.denied')));
 return;
 }
