...
class auth_vb {
	...
	function auth_vb() {
		...
		if (pluginGetVariable('auth_vb','extdb')) {
	...
	function login($auto_scan = 1, $username = '', $password = '') {
		...
		if (!pluginGetVariable('auth_vb', 'autocreate_ng')) {
			return '';
		}
	...
	function fetch_substr_ip($ip, $length = null)
	{
		if ($length === null OR $length > 3) {
			$length = pluginGetVariable('auth_vb','ipcheck');
		}
	...
	function check_auth() {
		...
		$gotuserid = 0;
		if ($sessionhash) {
			$query = "select * from ".$dbprefix."session where sessionhash=".db_squote($sessionhash)." and lastactivity > unix_timestamp(now()) - ".intval(pluginGetVariable('auth_vb','cookietimeout'))." and idhash = ".db_squote($session_idhash);
	...
	function drop_auth() {
		...
		foreach (array('bbsessionhash', 'bbuserid', 'bbpassword', 'zz_auth') as $c)
			if (isset($_COOKIE[$c]))
				@setcookie($c, '', time() - 3600 * 24 * 365, '/', pluginGetVariable('auth_vb','cookie_domain')?pluginGetVariable('auth_vb','cookie_domain'):'');
	 	return;
	}
