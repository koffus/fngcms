...
function plugin_tags_tag($params = [])
{
    ...
    $SYSTEM_FLAGS['info']['breadcrumbs'] = array(
        ...
        array('link' => generatePluginLink('tags', 'tag', array('tag' => $tag)),'text' => secure_html($tag)),
    ...
    if (!($rec = $mysql->record("select * from ".prefix."_tags where tag=".db_squote($tag)))) {
        ...
        if (pluginGetVariable('tags', 'tpage_paginator')) {
            ...
            $pageNo = isset($params['page']) ? intval($params['page']) : 1;
            if ($pageNo < 1)
                $pageNo = 1;
            ...
            $paginationParams = array('pluginName' => 'tags', 'pluginHandler' => 'tag', 'params' => array('tag' => $tag), 'xparams' => array(), 'paginator' => array('page', 0, false));
...
function plugin_tags_generatecloud($ppage = 0, $catlist = '', $age = 0, $params = [])
{
    ...
    if ( $ppage and (($pagesCount > 1) or ($page != 1))) {
        ...
        $tvars = array();
        $tvars['regx']["'\[prev-link\](.*?)\[/prev-link\]'si"] = ($page > 1)?str_replace('%page%',"$1",str_replace('%link%',generatePageLink($paginationParams, $page - 1), $navigations['prevlink'])):'';
        $tvars['regx']["'\[next-link\](.*?)\[/next-link\]'si"] = ($page < $pagesCount)?str_replace('%page%',"$1",str_replace('%link%',generatePageLink($paginationParams, $page + 1), $navigations['nextlink'])):'';
