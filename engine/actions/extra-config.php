...
if (!is_array($extras[$plugin])) {
	...
	if ( ($stype != 'install') and ($stype != 'deinstall') ) {
		$stype = 'config';
	}
	...
	if (is_array($extras[$plugin]) and ($extras[$plugin][$stype]) and is_file($cfg_file)) {
		...
		if (($stype == 'config') and (isset($_REQUEST['action']) and $_REQUEST['action'] == 'commit') and ($_REQUEST['token'] != genUToken('admin.extra-config'))) {
