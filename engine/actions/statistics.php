...
$note_path = root . 'trash/' . $parse->translit($userROW['name']) . '_note.inc.txt';
...
if ($action == 'save') {
	...
	if (!$note or $note == '') {
		...
		fwrite($fp, $note);
		fclose($fp);
		msg(array('message' => __('msgo_note_saved')));
	}
}
...
$tVars = array(
	...
	'comments' => $cPlugin->isInstalled('comments')?$mysql->result("SELECT count(id) FROM ".prefix."_comments"):'-',
...
foreach (array('register_globals', 'magic_quotes_gpc', 'magic_quotes_runtime', 'magic_quotes_sybase') as $flag) {
	$tVars['flags'][$flag] = ini_get($flag)?('<font color="red"><b>'.__('perror.on').'</b></font>'):__('perror.off');
