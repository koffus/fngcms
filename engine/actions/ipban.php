...
function ipban_add() {
	...
	if ( (!isset($_REQUEST['token'])) or ($_REQUEST['token'] != genUToken('admin.ipban')) ) {
	...
	if (preg_match('#^(\d+)\.(\d+)\.(\d+)\.(\d+)$#', $ip, $m) && ($m[1] < 256) && ($m[2] < 256) && ($m[3] < 256) && ($m[4] < 256)) {
		...
		if (!preg_match('#^1+0+$#', $lbmask, $null)) {
			msg(array('type' => 'danger', 'title' => __('ipban')['msge.mask'], 'message' => __('ipban')['msge.mask#desc']));
			return;
		}
		...
		if ($net_len > 16777216) {
			msg(array('type' => 'danger', 'title' => __('ipban')['msge.masklen'], 'message' => __('ipban')['msge.masklen#desc']));
			return;
		}
	}
	...
	if ($result) {
			...
			if (is_array($mysql->record("select addr from ".prefix."_ipban where addr_start=".db_squote(sprintf("%u", $addr_start))." and addr_stop=".db_squote(sprintf("%u", $addr_stop))))) {
				// Duplicated
				msg(array('type' => 'danger', 'message' => __('ipban')['msge.exist']));
				return;
			}
			...
			msg(array('message' => str_replace('{ip}', $ip, __('ipban')['msg.blocked'])));
	} else {
		msg(array('type' => 'danger', 'title' => __('ipban')['msge.fields'], 'message' => __('ipban')['msgi.fields']));
	}
}
...
function ipban_delete() {
	...
	if (is_array($row=$mysql->record("select * from ".prefix."_ipban where id = ".$id))) {
		...
		msg(array('message' => str_replace('{ip}', $row['addr'], __('ipban')['msg.unblocked'])));
	} else {
		msg(array('type' => 'danger', 'message' => __('ipban')['msg.notfound']));
	}
}
...
//
function ipban_list() {
	global $mysql, $mod, $twig, $PHP_SELF;
