...
function admCategoryList($retMode = 0) {
    global $mysql, $PHP_SELF, $twig, $config, $AFILTERS;

    // Check for permissions
    if (!checkPermission(array('plugin' => '#admin', 'item' => 'categories'), null, 'view')) {
        switch ($retMode) {
            case 1:
                return msg(array('type' => 'danger', 'message' => __('perm.denied')), 1, 2);
            case 2:
                return false;
            default:
...
function admCategoriesRPCmodify($params) {
    ...
    switch ($params['mode']) {
        ...
        case 'down':
            ...
            return (array('status' => 1, 'errorCode' => 0, 'errorText' => 'Ok', 'infoCode' => intval($moveResult), 'infoText' => '<img src="'.skins_url.'/images/'.$params['mode'].'.gif"/> '.$catz[$catmap[$params['id']]]['name'], 'content' => $data));
